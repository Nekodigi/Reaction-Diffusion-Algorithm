  ...
  for(int y = 1; y < (int)(height/gridInterval)+2; y++){
    ...
    for(int x = 1; x < (int)(width/gridInterval)+2; x++){
      ...
      if(random(0, 1) < 0.0000001){
        ...
        data.tier = random(100000, 1000000);
      }
      if(data.amount > 0){
      ...
      if(data.poplation < data.things){
        ...
        data.poplation = data.poplation*1.001+data.poplation*data.things/100000000;
      }
      else{
      ...
      fill((-(float)data.things/100000)*(float)gridInterval+250, 100, 100);
