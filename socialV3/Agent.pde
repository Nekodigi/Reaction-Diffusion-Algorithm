...
void Agentdraw(){
  ...
  for(Agent agent:agents){
    ...
    float angle = map(noise(agent.x/noiseScale, agent.y/noiseScale, azoff), 0, 1, -PI, PI);
    if(agent.x <= 0 || agent.x >= width || agent.y <= 0 || agent.y >= height){
      ...
      impulse((int)(agent.x/gridInterval), (int)(agent.y/gridInterval),1000000);
    }
    ...
    Nagent.col = color(sin((float)frameCount/12000)*100+100, sin((float)frameCount/120)*40+60, sin((float)frameCount/1200)*40+60+random(0, 10), 10);
