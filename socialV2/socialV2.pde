...
void impulse(int x, int y){
  Data data = datas[x][y];
  data.things = random(10000000, 100000000);
}
...
void draw(){
  ...
  float yoff = 0;;
  if(mouseButton == LEFT){
    impulse((int)(mouseX/gridInterval), (int)(mouseY/gridInterval));
  }
  ...
  for(int y = 0; y < (int)(height/gridInterval)+2; y++){
    ...
    for(int x = 0; x < (int)(width/gridInterval)+2; x++){
      ...
      fill((-(float)data.things/pow(10,thingsRange))*(float)gridInterval+250, 100, 100);
